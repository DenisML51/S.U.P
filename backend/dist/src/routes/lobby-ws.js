import { MemberStatus, MessageVisibility } from '@prisma/client';
...
export const lobbyWsRoutes = async (app) => {
    const lobbySockets = new Map();
    const removeConnection = (lobbyKey, socketRef) => {
    ...
    const emitLobbyEvent = (lobbyKey, event, filter = () => true) => {
        const pool = lobbySockets.get(lobbyKey);
        if (!pool) {
            return;
        }
    ...
    const syncStateForLobby = async (lobbyKey) => {
        const pool = lobbySockets.get(lobbyKey);
        if (!pool) {
            return;
        }
    ...
    app.get('/ws/lobby', { websocket: true }, async (socket, request) => {
        ...
        const pool = lobbySockets.get(lobbyKey) ?? new Set();
        pool.add(liveConn);
