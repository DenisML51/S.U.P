          ...
          >
            <div className="p-6 border-b border-dark-border flex items-center justify-between bg-dark-card/50 backdrop-blur-sm relative z-[100]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold">{resource ? t('resourceModal.editTitle') : t('resourceModal.newTitle')}</h2>
                  <p className="text-xs text-gray-400">{t('resourceModal.subtitle')}</p>
                </div>
              </div>
              <button 
                onClick={onClose} 
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-dark-bg/50 border border-dark-border rounded-xl">
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2 text-center">{t('resourceModal.current')}</label>
                  ...
                  <div className="p-3 bg-dark-bg/50 border border-dark-border rounded-xl">
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-2 text-center">{t('resourceModal.maximum')}</label>
                    <div className="flex items-center justify-between">
              ...
              <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-2xl space-y-4">
                <div className="flex items-center gap-2 text-blue-400">
                  <Wand2 size={16} />
                  <span className="text-xs font-bold uppercase tracking-wider">{t('resourceModal.magicFocus')}</span>
                </div>
                ...
                <CustomSelect
                  ...
                  options={[
                    { value: '-1', label: t('resourceModal.regularResource') },
                    { value: '0', label: t('resourceModal.slotsSpecial') },
                    ...Array.from({ length: 9 }, (_, i) => ({ value: (i + 1).toString(), label: `${t('resourceModal.slots')} ${i + 1} ${t('spellView.circle')}` }))
                  ]}
                />
                <p className="text-[10px] text-gray-500 leading-relaxed px-1">
                  {t('resourceModal.magicFocusHint')}
                </p>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                  ...
                  {t('resourceModal.effectDescription')}
                </label>
                <MarkdownEditor
                  value={description}
                  onChange={setDescription}
                  placeholder={t('resourceModal.effectPlaceholder')}
                  rows={4}
                  minHeight="120px"
                />
              </div>
