...
import { getConditionDescription, getConditionName } from '../../../i18n/domainLabels';
  ...
  return (
    <div className="bg-dark-card/40 backdrop-blur-md rounded-2xl border border-white/5 p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-white/60 uppercase tracking-wider">{t('conditions.title')}</h3>
        <button 
      ...
      <div className="flex flex-wrap gap-2">
        <AnimatePresence mode="popLayout">
          {activeConditions.map(condition => (
            ...
            >
              <span>{getConditionName(condition.id, condition.name, t)}</span>
              <button 
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 rounded-lg bg-dark-bg border border-white/10 text-xs text-white/70 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-2xl backdrop-blur-xl">
                {getConditionDescription(condition.id, condition.description, t)}
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
        ...
        {activeConditions.length === 0 && !showPicker && (
          <div className="text-sm text-white/20 italic py-1">{t('conditions.empty')}</div>
        )}
      </div>
            ...
            <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 gap-2">
              {CONDITIONS.map(condition => {
                ...
                return (
                  ...
                  >
                    {getConditionName(condition.id, condition.name, t)}
                  </button>
                );
              })}
            </div>
