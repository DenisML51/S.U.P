...
export const PlayersCombatSidebar: React.FC = () => {
  ...
  const sourceMembers = (combatMembers.length ? mergedMembers : members) as any[];
  ...
  return (
    <aside className="fixed left-3 top-[130px] z-[46] w-[300px] rounded-2xl border border-white/10 bg-dark-bg/85 p-3 shadow-2xl backdrop-blur-xl">
            ...
            >
              <div className="mb-1 grid grid-cols-[auto_1fr] gap-2">
                <div className="flex items-center gap-2">
                  {avatar ? (
                    <img src={avatar} className="h-8 w-8 rounded-md object-cover border border-white/20" />
                  ) : (
                    <div className="h-8 w-8 rounded-md bg-white/10 border border-white/20" />
                  )}
                </div>
                <div>
                  ...
                  <div className="mt-1 text-gray-300">HP: {hp} / {maxHp}</div>
                  <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-black/40 border border-white/10">
              ...
              <div className="mt-2 grid grid-cols-3 gap-1 text-[10px]">
                <div className="rounded border border-blue-500/30 bg-blue-500/10 p-1">
                  <div className="mb-1 text-blue-300">Осн.</div>
                  <div className="flex gap-1">{renderActionDots(actionLimits.action, spentActions.action, '#60a5fa')}</div>
                </div>
                <div className="rounded border border-green-500/30 bg-green-500/10 p-1">
                  <div className="mb-1 text-green-300">Бонус</div>
                  <div className="flex gap-1">{renderActionDots(actionLimits.bonus, spentActions.bonus, '#4ade80')}</div>
                </div>
                <div className="rounded border border-orange-500/30 bg-orange-500/10 p-1">
                  <div className="mb-1 text-orange-300">Реакц.</div>
                  <div className="flex gap-1">{renderActionDots(actionLimits.reaction, spentActions.reaction, '#fb923c')}</div>
                </div>
              </div>
              <div className="mt-1 text-gray-400">Действие: {formatAction(member.currentAction)}</div>
            </div>
          );
        })}
      </div>
