...
export const GrimmoireModal: React.FC<GrimmoireModalProps> = ({
  ...
  const allSchools = [
    t('grimmoire.school.evocation'),
    t('grimmoire.school.conjuration'),
    t('grimmoire.school.illusion'),
    t('grimmoire.school.necromancy'),
    t('grimmoire.school.enchantment'),
    t('grimmoire.school.transmutation'),
    t('grimmoire.school.divination'),
    t('grimmoire.school.abjuration'),
  ];
          ...
          >
            <div className="p-6 border-b border-white/5 bg-dark-card/50 backdrop-blur-xl flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-2xl font-black text-white leading-none">{t('grimmoire.title')}</h2>
                  <p className="text-xs text-gray-500 font-bold uppercase tracking-widest mt-1">{t('grimmoire.subtitle')}</p>
                </div>
              </div>
                ...
                <div className="flex items-center gap-4 sticky top-0 z-10 bg-dark-card/80 backdrop-blur-md py-2">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      placeholder={t('spellsTab.searchByName')}
                    ...
                    <div key={group.level} className="space-y-4">
                      <div className="flex items-center gap-4">
                        <div className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] whitespace-nowrap">
                          {group.level === 0 ? t('spellView.cantripPlural') : `${group.level} ${t('spellView.circle')}`}
                        </div>
                        <div className="h-px flex-1 bg-gradient-to-r from-blue-500/20 to-transparent" />
                        <div className="text-[10px] font-bold text-gray-500 uppercase">
                          {t('grimmoire.prepared')}: {getPreparedCount(group.level)} / {getMaxPrepared(group.level)}
                        </div>
                      </div>
                              ...
                              <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-4 flex-1">
                                  <button
                                    ...
                                    title={!knowsSchool ? t('grimmoire.schoolUnknown') : !canPrep && !spell.prepared ? t('grimmoire.prepareLimitReached') : spell.prepared ? t('grimmoire.unprepare') : t('grimmoire.prepare')}
                                  >
                    ...
                    <div className="p-6 space-y-8">
                      ...
                      <div>
                        <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                          ...
                          {t('grimmoire.prepareLimits')}
                        </h3>
                        <div className="space-y-3">
                          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(level => (
                            <div key={level} className="flex items-center justify-between bg-dark-bg/50 p-2 rounded-xl border border-white/5">
                              <span className="text-[10px] font-bold text-gray-400 uppercase ml-2">{level} {t('spellView.circle')}</span>
                              <div className="flex items-center gap-3">
                                <button 
